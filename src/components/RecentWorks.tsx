  ...
  return (
    <div className={`relative md:h-[579px] h-[289px] w-full md:first:col-span-3 md:col-span-2 ${image} bg-no-repeat md:hover:first:bg-[length:160%_100%] md:hover:last:bg-[length:160%_75%] md:first:bg-[length:80%_50%] md:last:bg-[length:80%_35%] bg-[length:80%_60%] transition-[background-size] ease-in-out bg-center cursor-pointer group`} onClick={() => router.push(case_study)}>
      ...
      <div className="relative h-full text-white p-4 flex flex-col justify-between">
        <div>
          <p className="text-base md:text-2xl font-bold tracking-wider">{title}</p>
          <p className="text-sm">{desc}</p>
        </div>
        <div className="flex gap-6">
          <Link href={github} className="py-2 px-5 bg-portfolio-black font-semibold rounded-full hover:text-portfolio-hero-yellow">
            Github
          </Link>
          <Link href={live_link} className="py-2 px-5 bg-portfolio-black font-semibold rounded-full hover:text-portfolio-hero-yellow">
            Live
          </Link>
        </div>
      </div>
