...
function Card({ image, github, live_link, case_study, desc, title }: { image: string; github: string; live_link: string; case_study: string; desc: string; title: string }) {
  return (
    <Link className={`relative h-[579px] w-full md:first:col-span-3 md:col-span-2 ${image} bg-no-repeat hover:first:bg-[length:160%_100%] hover:last:bg-[length:160%_75%] first:bg-[length:80%_50%] last:bg-[length:80%_35%] transition-[background-size] ease-in-out bg-center cursor-pointer`} href={case_study}>
